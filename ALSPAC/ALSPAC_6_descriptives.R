...
pdf(file.path(descpath, paste0('ALSPAC_0_hist_original_sample_',date,'.pdf')))
...
# Sample selection =============================================================
sink(file.path(descpath, paste0('ALSPAC_0_flowchart_selection_',date,'.txt')))
...
sample <- samp3[!is.na(samp3$sibling) & samp3$sibling==0,]; # 8 missing values cause a problem cause set all variables to missing
...
# Save sample as file
saveRDS(sample, file.path(datapath, 'raw_data_sample.rds'))

# Save histograms of all variables in the selected sample
pdf(file.path(descpath, paste0('ALSPAC_0_hist_selected_sample_',date,'.pdf')))
for (var in names(data)[-which(names(data) %in% c('IDC','sleep_bedtime', 'sleep_waketime'))]) { # these cannot be converted to numeric
...
for (v in names(data)[-which(names(data)%in%c('IDC'))]) { summary_df = rbind(summary_df, summ_to_df(v)) } 

# write.csv(summary_df, file.path(descpath, paste0('1_summary_pre_imp_',date,'.csv')))

# FULL AND SELECTED SAMPLE (AFTER IMPUTATION ) =================================

describe <- function(imput=imput) {
...
openxlsx::write.xlsx(c(list('missing_pattern'=miss,'s_pre-imp'=summary_df), s), 
                     file = file.path(descpath, paste0('ALSPAC_Descriptives_',date,'.xlsx')), 
                     overwrite=T)
