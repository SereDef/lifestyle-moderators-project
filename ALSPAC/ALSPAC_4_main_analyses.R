...
pool_fit <- function(mods=c('exerc','sleep','mdiet'), z_or_bin='_z', interact='*',
                     outc='comorb', exp='ELS_z', sensitive=F) {
  cat(outc, '\n')
  # Define covariates
  ...
  for (modr in paste0(mods,z_or_bin)) {
    if (sensitive) {
      cat(nrow(sample_mids$data),'-->',nrow(sample_mids$data[!is.na(sample_mids$data$comorb),]))
      sample <- mice::filter(sample_mids, !is.na(sample_mids$data$comorb) & !is.na(sample_mids$data[modr]))
    } else { sample <- sample_mids }

    N = nrow(sample$data); cat('-->', N, '\n')
    # IF outcome is comorbidity group fit a multimod model else lm
    if (startsWith(outc, 'comorb')) {
      fit <- with(sample, nnet::multinom(as.formula(paste(outc,'~',exp, interact, modr, covs)),
                                              model=T, trace=F));
    } else {
      fit <- with(sample, lm(as.formula(paste(outc,'~',exp, interact, modr, covs)))); }
...
# Sensitivity ------------------------------------------------------------------
els_com <- pool_fit(sensitive=T)

openxlsx::write.xlsx(els_com, file = file.path(respath, paste0('ALSPAC_SensyResults_',date,'.xlsx')),
                     overwrite=T)
...
fit_spline <- function(outc) {
  ...
  for (mod in c('exerc','sleep','mdiet')) {
    ...
    # Initiate dataframe
    spl <- data.frame(row.names = mod.grid); ps2 = list(); ps3 = list()
    ...
    for (i in 0:30) {
      ...
      fit <- lm(as.formula(paste0(outc,' ~ splines::ns(',mod,',4)')), data=imp_i)

      f0 <- lm(as.formula(paste(outc,'~ ELS_z *', mod, covs)), data=imp_i)
      f1 <- lm(as.formula(paste(outc,'~ ELS_z * poly(', mod,',2, raw=T)', covs)), data=imp_i)
      f2 <- lm(as.formula(paste(outc,'~ ELS_z * poly(', mod,',3, raw=T)', covs)), data=imp_i)

      ps2 <- cbind(ps2, anova(f1,f0)[2,'Pr(>F)'])
      ps3 <- cbind(ps3, anova(f2,f0)[2,'Pr(>F)'])

      pred = predict(fit, newdata = nd, se.fit=T)
    ...
    spl_list[[paste0(substr(outc,1,6),'_',mod,'_Pval_poly2')]] <- ps2
    spl_list[[paste0(substr(outc,1,6),'_',mod,'_Pval_poly3')]] <- ps3
...
# ==============================================================================
# PREVENTATIVE MEDICINE REVISION ===============================================
# ==============================================================================
pack <- c('mice','miceadds','nnet','car','openxlsx')
invisible(lapply(pack, require, character.only = T));

# ==> point to the directory containing the file "imp_sample.rds"
# *SHOULD* be X:/Psychology/ResearchProjects/EWalton/ (EarlyCause/WP3/SD/Data)
datapath <- dirname(file.choose())
respath <- paste0(datapath, '/../Results')
sample_mids <- readRDS(file.path(datapath,'imp_sample.rds'))

# Copied from main analyses
...
# Check VIF accounting for interactions
ogdat <- complete(sample_mids, 0)

vif_check <- list()
for (mod in c('exerc','sleep','mdiet')) {
  f <- paste0('as.integer(comorb) ~ ELS_z * ',mod,'_z + age_child + sex + m_bmi_before_pregnancy + ethnicity + m_smoking + m_drinking')
  test <- lm(as.formula(f), data = ogdat)
  # Calculate the VIF for each predictor
  vifdf <- car::vif(test, type='predictor')
  vif_check[[paste0(mod,'_VIF')]] <- vifdf
}

# Maternal BMI interactions...
# Z -score for interpretable terms
sample_mids <- datlist2mids( scale_datlist( mids2datlist(sample_mids),
                                            orig_var = 'm_bmi_before_pregnancy',
                                            trafo_var = 'm_bmi_prepregn_z'))

# Run model
mbmi_com <- pool_fit(mods=c('m_bmi_prepregn'), z_or_bin='_z', interact='*',
                     outc='comorb',
                     exp='ELS_z',
                     covs='+ age_child + sex + ethnicity + m_smoking + m_drinking')

# Save output
modls <- append(list('mbmi_com'= mbmi_com), vif_check)

date <- format(Sys.Date(),'%d%m%y')

openxlsx::write.xlsx(modls, file = file.path(respath, paste0('ALSPAC_Revision1_',date,'.xlsx')),
                     overwrite=T)
